...
const apiFetchquickSearch = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/quickSearch?scenario=${encodeURIComponent(searchQuery)}`, {
        method: "GET",
        headers: {
      ...
      if (data.length > 0) {
        setResult(data); // Assuming the API returns an array of tips
      } else {
...
export default apiFetchquickSearch;
